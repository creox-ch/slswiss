...
| **✅ Mobile adaptive (2026-07-01)** | `mobile-adaptive` — первый `@media` в проекте: гамбургер-меню + hero/секции/сетки → мобильная раскладка (≤768px). CI зелёный (блок 17) |
...
## История STATE.md
- **2026-07-01** — `mobile-adaptive` закрыт: добавлен первый в проекте `@media (max-width:768px)`
  (раньше их было 0). Гамбургер-меню (☰) в шапке, hero → 1 колонка, секции/футер меньше отступов,
  многоколоночные сетки → 1–2 колонки, hero-шапки страниц ужаты (убран горизонтальный скролл).
  E2E `tests/17-*` (viewport 390) зелёные. Chrome-MCP из Cowork мобилку не рендерит — проверка Playwright/телефоном.
