...
| | |
|---|---|
| **Текущий спринт** | _нет активного_ — выбрать следующий (разблокированы `catalog-moderation`/`events-moderation`) |
| **✅ Готово (2026-06-29)** | `registration-fix` (Resend SMTP) · `resend-smtp` · `forms-mvp-backend` (формы → Supabase) |
