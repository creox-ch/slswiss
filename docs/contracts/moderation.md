...
- **2026-07-01** — доработка: панель показывает 3 секции — «На модерации» (одобрить/отклонить),
  «Опубликовано» (**снять с публикации** → rejected), «Скрытые» (**вернуть** → approved). Модератор
  может выключать уже опубликованные бизнесы/события из каталога и возвращать их. Тест 13.3.
  Без миграции (переиспользуем статус `rejected`).
