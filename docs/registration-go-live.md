...
**Создан:** 2026-06-29
**Статус:** ✅ **ВЫПОЛНЕНО 2026-06-29** — регистрация работает end-to-end. Остаток (не-блокер):
письма в Спам → задача `email-deliverability` в BACKLOG.
**Цель:** довести `registration-fix` до рабочего состояния — письма подтверждения реально
доходят, клик по ссылке логинит пользователя на сайте.
**Решения:** провайдер — **Resend**, отправитель — **noreply@slswiss.ch**, домен — **slswiss.ch**.

> ⚠ Ключевой урок: поле **Site URL** в Supabase роняло `redirect_to` писем на корень GitHub Pages
> (404), даже когда показывало `/slswiss/`. Решено в коде — `emailRedirectTo: authRedirectURL()`
> (текущая страница) в `sbSignUp`/`resendConfirmation`. Адрес уже в allow-list Redirect URLs.
